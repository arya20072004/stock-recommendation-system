...
def get_stocks_summary():
    ...
    try:
        ...
        logger.error(f"Database error in /api/stocks/summary: {e}")
        return jsonify({'error': 'Database failure'}), 503
    except Exception as e:
        logger.error(f"Unexpected error in /api/stocks/summary: {e}")
    ...
    try:
        ...
        if pred_doc:
            prediction_obj = {
                ...
                "provenance_hash": pred_doc.get("provenance_hash"),
                "target_return_threshold": pred_doc.get("target_return_threshold"),
                "top_features": []
            }
