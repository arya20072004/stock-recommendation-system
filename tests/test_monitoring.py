...
def test_fetch_evaluated_ignores_pending_and_legacy(mock_db):
    mock_db.prediction_history.find.return_value = MockCursor([
        {"status": "EVALUATED", "target_return_threshold": 0.02, "market_date": "2026-08-01", "symbol": "A"},
    ])
    
    preds = fetch_evaluated_predictions(mock_db, {})
    assert len(preds) == 1
    assert preds[0]["market_date"] == "2026-08-01"
    
    # Check that the query dict contains the correct filters
...
def test_ticker_and_model_version_separation(mock_db):
    mock_db.prediction_history.find.return_value = MockCursor([
        {"status": "EVALUATED", "target_return_threshold": 0.02, "market_date": "2026-08-01", "symbol": "TCS.NS", "model_version": "v1", "recommendation": "BUY", "actual_class": "BUY"},
        {"status": "EVALUATED", "target_return_threshold": 0.02, "market_date": "2026-08-02", "symbol": "TCS.NS", "model_version": "v1", "recommendation": "BUY", "actual_class": "SELL"}
    ])
...
def test_rolling_50_window_behavior(mock_db):
    ...
    docs = []
    for i in range(60):
        docs.append({"status": "EVALUATED", "target_return_threshold": 0.02, "market_date": f"2026-01-{i:02d}", "symbol": "TCS.NS", "recommendation": "BUY", "actual_class": "BUY"})
...
def test_health_state_reasoning(mock_db):
    ...
    mock_db.prediction_history.find.return_value = MockCursor([
        {"status": "EVALUATED", "target_return_threshold": 0.02, "market_date": "2026-08-01", "symbol": "A", "recommendation": "BUY", "actual_class": "BUY"}
    ] * 29)
    ...
    mock_db.prediction_history.find.return_value = MockCursor([
        {"status": "EVALUATED", "target_return_threshold": 0.02, "market_date": "2026-08-01", "symbol": "C", "recommendation": "BUY", "actual_class": "SELL"}
    ] * 61 + [
        {"status": "EVALUATED", "target_return_threshold": 0.02, "market_date": "2026-08-01", "symbol": "C", "recommendation": "BUY", "actual_class": "BUY"}
    ] * 39)
    health = get_system_health(mock_db)["health"]
    assert health["state"] == "HEALTHY"
    ...
    mock_db.prediction_history.find.return_value = MockCursor([
        {"status": "EVALUATED", "target_return_threshold": 0.02, "market_date": "2026-08-01", "symbol": "B", "recommendation": "BUY", "actual_class": "BUY"}
    ] * 4 + [
        {"status": "EVALUATED", "target_return_threshold": 0.02, "market_date": "2026-08-01", "symbol": "B", "recommendation": "HOLD", "actual_class": "HOLD"}
    ] * 96)
    health = get_system_health(mock_db)["health"]
    assert health["state"] == "HEALTHY"
