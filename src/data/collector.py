...
def run(target_date=None):
    ...
    for ticker in TICKERS:
        ...
        try:
            ...
            if records_to_insert:
                ...
                for record in records_to_insert:
                    ...
                    operations.append(UpdateOne(
                        ...
                        [{
                            '$set': {
                                'open': {'$cond': [{'$eq': [{'$ifNull': ['$source', '']}, 'NSE_BHAVCOPY']}, '$open', record['open']]},
                                'high': {'$cond': [{'$eq': [{'$ifNull': ['$source', '']}, 'NSE_BHAVCOPY']}, '$high', record['high']]},
                                'low': {'$cond': [{'$eq': [{'$ifNull': ['$source', '']}, 'NSE_BHAVCOPY']}, '$low', record['low']]},
                                'close': {'$cond': [{'$eq': [{'$ifNull': ['$source', '']}, 'NSE_BHAVCOPY']}, '$close', record['close']]},
                                'volume': {'$cond': [{'$eq': [{'$ifNull': ['$source', '']}, 'NSE_BHAVCOPY']}, '$volume', record['volume']]},
                                'source': {'$cond': [{'$eq': [{'$ifNull': ['$source', '']}, 'NSE_BHAVCOPY']}, 'NSE_BHAVCOPY', 'YAHOO']}
                            }
                        }],
                        upsert=True
                    ))
            ...
            range_end = pd.Timestamp(target_date).normalize() if target_date else data.index.max()
