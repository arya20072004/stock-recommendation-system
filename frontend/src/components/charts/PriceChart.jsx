...
export function PriceChart({ chartData, range, onRangeChange, loading, direction = 'positive' }) {
  const cssColor = direction === 'negative' ? 'var(--negative)' : 'var(--positive)'
              ...
              <AreaChart data={chartData} margin={{ top: 10, right: 4, left: -22, bottom: 0 }}>
                <defs>
                  <linearGradient id="stock-price-area" x1="0" x2="0" y1="0" y2="1">
                    <stop offset="0%" stopColor={cssColor} stopOpacity={0.24} />
                    <stop offset="100%" stopColor={cssColor} stopOpacity={0} />
                  </linearGradient>
                </defs>
                <CartesianGrid vertical={false} stroke="var(--border-primary)" strokeDasharray="3 5" />
